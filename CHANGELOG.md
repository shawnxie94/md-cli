...
- **`md-cli render` 命令**：新增本地 Markdown 渲染为微信公众号兼容 HTML 的能力，支持主题、字体、字号、主色、标题样式、代码主题、图注等多种样式参数及别名。
- **KaTeX passthrough 模式**：为 `md-cli render` 增加 KaTeX passthrough 选项，便于在 Node 渲染场景下保留数学公式原文以便下游处理。
